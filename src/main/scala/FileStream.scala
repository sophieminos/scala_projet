...
object FileStream extends ZIOAppDefault {
    ...
    override val run:ZIO [Any & ZIOAppArgs & Scope, Throwable, Unit] =
        ...
        //val result: ZIO[Console, Throwable, List[DensityPoint]] = 
        for {
            url <- ZIO.succeed(getClass.getClassLoader.getResource(resourcePath))
            reader <- ZIO.succeed(CSVReader.open(url.getFile()))
            densityPoints <- ZStream
            .fromIterator[Seq[String]](reader.iterator)
            .take(4)
